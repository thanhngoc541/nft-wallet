    import { EventDispatcher, Vector2, Vector3, Matrix4, Mesh, MeshBasicMaterial, Box3, BufferGeometry, Plane } from 'three';
     ...
     *
     * Once rooms have been identified, Half Edges are created for each 1
     *
    ...
    export class HalfEdge extends EventDispatcher {
        ...
        __generateEdgePlane(interior = true, plane = null) {
            ...
            let points = this.__vertices
    
            points.push(new Vector3(2, 1, 0))
            points.push(new Vector3(3, 2, 0))
            ...
            if (!plane) {
                plane = new Mesh(geometry, new MeshBasicMaterial({ visible: true }));
            } else {
                plane.geometry.dispose();
                plane.geometry = geometry; //this.__plane.geometry.fromGeometry(geometry);
            }
        ...
        distanceTo(x, y) {
            if (this.wall.wallType === WallTypes.STRAIGHT) {
                ...
                let p = this.wall.bezier.project({ x: x, y: y });

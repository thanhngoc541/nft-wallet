...
import FloorModel from "../model-3d/floor";
...
export const ConvertJson2DTo3D = (data) => {
    ...
    let points = [];
    floorCorners.forEach((cornerId) => {
        points.push(new Vector2(corners[cornerId].x, corners[cornerId].y))
    })

    let floor3D = new FloorModel(points, null)
    // sàn nhà
    // walls3D.push(floor3D);

    /** Todo:
     * Sửa wall thành shape geometry
     *  */
    // let Oxy = [], Oyz = [];
    //

...
import {EVENT_DELETED, EVENT_MOVED, EVENT_UPDATED} from "../../core/events";
import {
    Configuration,
    directionalDrag,
    dragOnlyX,
...
export class WallDimensions extends Graphics {
...
export class Edge extends BaseFloorplanViewElement {
...
export class WallView extends BaseFloorplanViewElement {
    constructor(floorplan, options, wall) {
        super(floorplan, options);
        ...
        this.__info = new WallDimensions(floorplan, options, wall);
        ...
        if (wall.backEdge) {
            this.__backEdge = new Edge(this.__floorplan, options, this.__wall, wall.backEdge);
            this.addChild(this.__backEdge);
        }
        if (wall.frontEdge) {
            this.__frontEdge = new Edge(this.__floorplan, options, this.__wall, wall.frontEdge);
    ...
    __dragMove(evt) {
        ...
        if (this.__isDragging) {
            ...
            if (Configuration.getValue(directionalDrag, 2)) {
                let projected = this.__wall.projectOnWallPlane(cmCo);
                let vectorToProjected = cmCo.clone().sub(projected);

                if (Configuration.getValue(snapToGrid, 2) || this.__snapToGrid) {
                    let snappedLength = Math.floor(vectorToProjected.length() / Configuration.getValue(snapTolerance, 2)) * Configuration.getValue(snapTolerance, 2);

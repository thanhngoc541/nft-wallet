...
class CornerGroupTransformationPoint extends Sprite {
    ...
    __dragMove(evt) {
        if (this.__isDragged) {
            evt.stopPropagation();
            let snapping = Configuration.getValue(snapToGrid, 3);
            let co = evt.data.getLocalPosition(this.parent);

            if (snapping) {
                co.x = Math.floor(co.x / Configuration.getValue(snapTolerance, 2)) * Configuration.getValue(snapTolerance, 2);
                co.y = Math.floor(co.y / Configuration.getValue(snapTolerance, 2)) * Configuration.getValue(snapTolerance, 2);
            }
...
export class CornerGroupTransform extends Graphics {

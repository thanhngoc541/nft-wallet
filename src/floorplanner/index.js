...
import {KeyboardListener} from "./viewer/KeyboardListener";
import {Grid} from "./viewer/Grid";
import {CornerGroupTransform} from "./viewer/CornerGroupTransform";
import {BoundaryView} from "./viewer/BoundaryView";
import {Configuration, snapToGrid, snapTolerance, viewBounds} from "../core/configuration";
import {WallView} from "./viewer/WallView";
import {CornerView} from "./viewer/CornerView";
import {RoomView} from "./viewer/RoomView";
import {IS_TOUCH_DEVICE} from "../helpers/DeviceInfo";
...
export class Viewer extends Application {
    constructor(canvasHolder, floorplan, options) {
        ...
        console.log('canvasHolder: ', canvasHolder);
        console.log('real canvas: ', this.__canvasHolder)
        ...
        this.__switchModeBetweenDrawAndMoveEvent = this.__switchModeBetweenDrawAndMove.bind(this);
        ...
        this.__keyboard = new KeyboardListener();
        ...
        this.__grid2d = new Grid(this.view, options);
        this.__boundaryRegion2D = null;
        this.__groupTransformer = new CornerGroupTransform(this.__floorplan, this.__options);
        ...
        this.__floorplanContainer.on('dblclick', this.__switchModeBetweenDrawAndMoveEvent);
    ...
    __drawBoundary(){
        ...
        if(this.__floorplan.boundary){
            if(this.__floorplan.boundary.isValid){
                this.__boundaryRegion2D = new BoundaryView(this.__floorplan, this.__options, this.__floorplan.boundary);
    ...
    __selectionMonitor(evt) {
        ...
        if (evt.item) {
            ...
            } else if (evt.item instanceof RoomView) {
                item = evt.item.room;
    ...
    __switchModeBetweenDrawAndMove(evt) {
        console.log("double click")
        if (this.__mode === floorplannerModes.DRAW) {
            console.log("mode draw")
            this.switchMode(floorplannerModes.MOVE)
            console.log("current mode is ", this.__mode)
        }
    }
    ...
    __center(){
        ...
        let bounds = Dimensioning.cmToPixel(Configuration.getValue(viewBounds, 2)) * zoom;
    ...
    __redrawFloorplan() {
        ...
        for (i = 0; i < rooms.length; i++) {
            ...
            let roomView = new RoomView(this.__floorplan, this.__options, modelRoom);
        ...
        for (i = 0; i < this.__floorplan.walls.length; i++) {
            ...
            let wallView = new WallView(this.__floorplan, this.__options, modelWall);
        ...
        for (i = 0; i < this.__floorplan.corners.length; i++) {
            ...
            let cornerView = new CornerView(this.__floorplan, this.__options, modelCorner);
